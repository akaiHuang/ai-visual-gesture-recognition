...
AI Visual Gesture Recognition 是一套即時手部追蹤與手勢分類系統，將攝影機畫面轉為可用於互動的手勢事件。適合用於 HCI 原型、互動裝置控制與展示型應用，快速驗證以視覺手勢取代滑鼠/鍵盤輸入的操作流程。

## About (EN)

AI Visual Gesture Recognition is a real-time hand tracking and gesture classification system that converts camera input into actionable interaction events. It is suitable for HCI prototypes, touchless control interfaces, and interactive demos.
